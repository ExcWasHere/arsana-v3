...
import { Menu, BookOpen, Gamepad2, Award, Users, GraduationCap, Home, LogOut, User, ChevronLeft, ChevronRight, Check, Play } from 'lucide-react';
...
const Sidebar = ({ isOpen }: { isOpen: boolean }) => {
  ...
  useEffect(() => {
    const raw = localStorage.getItem('arsana_user');
    if (raw) {
  ...
  return (
    ...
    >
      ...
      <button
        className="py-2 rounded-lg font-bold flex items-center justify-center gap-2 transition-colors mb-6 text-xs border-2 bg-gradient-to-r from-cyan-500 to-teal-500 text-white border-transparent shadow-md hover:shadow-lg"
      >
        <User size={14}/> 
        Profil Saya
      </button>
      ...
      <div className="pt-4 border-t border-cyan-200 space-y-1">
        <button
          onClick={() => {
            localStorage.removeItem('arsana_user');
...
export default function ArsanaDashboard() {
  ...
  return (
    <div className="flex min-h-screen" style={{ 
      ...
      <main
        className={`flex-1 transition-all duration-300 ease-in-out p-6 lg:p-10 ${
      ...
      >
        <div className="flex items-center gap-4 mb-10">
          <button
            ...
            className="p-2.5 bg-white rounded-lg border border-cyan-200 hover:bg-cyan-50 transition-all group shadow-sm"
          >
            <Menu size={20} className="text-cyan-600 group-hover:text-teal-600" />
          </button>
          <div>
            <h1 className="text-2xl font-bold tracking-tight bg-gradient-to-r from-cyan-600 to-teal-600 bg-clip-text text-transparent uppercase">
              Beranda Arsana
            </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {/* Subjects Grid */}
            <div className="bg-white rounded-xl p-8 border border-cyan-200 shadow-sm">
              <h2 className="text-gray-800 font-bold text-lg uppercase tracking-wider mb-6 flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
                {subjects.map((subject, idx) => (
                  <div key={idx} className="group cursor-pointer">
                    <div className={`relative aspect-square rounded-xl overflow-hidden bg-gradient-to-br ${subject.color} p-6 flex flex-col items-center justify-center text-white shadow-lg hover:shadow-xl transition-all`}>
                      ...
                      <p className="text-xs mt-2 text-white/90">{subject.completed}/{subject.total} materi</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white rounded-xl p-8 border border-cyan-200 shadow-sm">
              <h2 className="text-gray-800 font-bold text-lg uppercase tracking-wider mb-6 flex items-center gap-2">
                <Gamepad2 size={20} className="text-teal-500" />
                ARENA PINTAR
              </h2>

              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {games.map((game, idx) => (
                  <div key={idx} className={`${game.color} rounded-xl p-6 text-white text-center cursor-pointer hover:scale-105 transition-transform shadow-md`}>
